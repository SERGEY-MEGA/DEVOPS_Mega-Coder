...
**Репозитории:** [GitLab `gitlub.ru/MEGA/deveps-mega-coder`](https://gitlub.ru/MEGA/deveps-mega-coder) · [GitHub `SERGEY-MEGA/DEVOPS_Mega-Coder`](https://github.com/SERGEY-MEGA/DEVOPS_Mega-Coder/tree/main)  
...
- В клоне репозитория: **`TELEGRAM_ALERTING_FULL_REPORT.md`** в корне (рядом с `README.md`).
- На GitHub: после **push** откройте файл в браузере — картинки подтягиваются из ветки `main` по путям `docs/screenshots/*.png`.  
  Если на GitHub файла нет — выполните `git add TELEGRAM_ALERTING_FULL_REPORT.md && git commit && git push` в [оба remotes](https://github.com/SERGEY-MEGA/DEVOPS_Mega-Coder/tree/main), как вы принято для [GitLab](https://gitlub.ru/MEGA/deveps-mega-coder).
...
6. **Связка с мониторингом**  
   ...
   - В GitLab (Settings → Webhooks) указывают URL `.../webhook/gitlab` и тот же секрет в заголовке.
...
Кратко по [docs/alerting/SETUP_LOG.md](./docs/alerting/SETUP_LOG.md):
...
|---------------------|------------|
| [docs/evidence/telegram-tests-live.txt](docs/evidence/telegram-tests-live.txt) | 2026-04-13: firing, resolved, GitLab, reporter — `sent=true` |
| [docs/evidence/alert-bot-logs-live.txt](docs/evidence/alert-bot-logs-live.txt) | Логи: `Telegram notification sent`, успешный POST webhook |
| [docs/evidence/reporter-live.md](docs/evidence/reporter-live.md) | Фрагмент Markdown-отчёта |
...
Файлы лежат в **`docs/screenshots/`** в репозитории. Ниже каждый снимок вынесен **отдельным блоком** — так GitHub и GitLab корректно показывают картинки в Markdown (в таблицах изображения часто ломаются).

### 10.1 Grafana — правила алертов (PrometheusRule в UI)

![Grafana: правила алертов](docs/screenshots/grafana-alert-rules.png)

### 10.2 Alertmanager — маршрут и receiver до webhook

![Alertmanager: конфигурация receiver](docs/screenshots/alertmanager-config.png)

### 10.3 Событие GitLab → сообщение в Telegram

![GitLab webhook: уведомление в Telegram](docs/screenshots/gitlab-event-message.png)

### 10.4 Отчёт / сводка в Telegram (reporter)

![Отчёт в Telegram](docs/screenshots/report-in-telegram.png)

### 10.5 Kubernetes — поды и статусы на стенде

![kubectl / поды и деплойменты](docs/screenshots/k8s-pods-status.png)

### 10.6 Веб-интерфейс приложения (контекст стенда)

![Приложение: фронт и бэкенд](docs/screenshots/app-frontend-backend.png)

### 10.7 Дополнительно (по желанию для защиты)

Имеет смысл добавить в `docs/screenshots/` собственные снимки **из приложения Telegram** с текстом `Alertmanager: СРАБАТЫВАЕТ` и `ВОССТАНОВЛЕНО` и вставить их сюда — см. [docs/screenshots/README.md](docs/screenshots/README.md).
...
- [docs/alerting/SETUP_LOG.md](./docs/alerting/SETUP_LOG.md) — журнал внедрения.  
- [docs/alerting/REPORT_ALERTING.pdf](./docs/alerting/REPORT_ALERTING.pdf) — PDF для сдачи.

---

**Итог:** бот создаётся через BotFather; в кластере работает сервис **`alert-bot`**, который по webhooks отправляет в Telegram алерты, события GitLab и отчёты reporter. Полный путь настройки и проверки — в [BOT_SETUP.md](./BOT_SETUP.md) и разделах выше. Чтобы отчёт с фото был виден на **GitHub**, файл `TELEGRAM_ALERTING_FULL_REPORT.md` и папка `docs/screenshots/` должны быть закоммичены и запушены в [репозиторий](https://github.com/SERGEY-MEGA/DEVOPS_Mega-Coder/tree/main).
